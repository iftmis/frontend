...
import { ObjectiveListPage } from './objective-list.po';
import { SidenavPage } from '../sidenav.po';
import { ObjectiveDetailPage } from './objective-detail.po';
import { ObjectiveDeletePage } from './objective-delete.po';

describe('Objective tests', () => {
  let headerPage: HeaderPage;
  let loginPage: LoginPage;
  let sidenavPage: SidenavPage;
  let listPage: ObjectiveListPage;
  let detailPage: ObjectiveDetailPage;
  let deletePage: ObjectiveDeletePage;
  ...
  beforeEach(() => {
    headerPage = new HeaderPage();
    sidenavPage = new SidenavPage();
    listPage = new ObjectiveListPage();
    detailPage = new ObjectiveDetailPage();
    deletePage = new ObjectiveDeletePage();
  });

  beforeEach(async () => {
    await waitAndClick(headerPage.appMenu);
    await waitAndClick(sidenavPage.objectiveMenu);
  });
  ...
  it('should display the objectives list page', async () => {
    expect(await listPage.getPageTitleText()).toEqual('Objectives');
  ...
  it('should create a new objective', async () => {
    ...
    expect(await detailPage.pageTitle.getText()).toEqual('Objective');
  ...
  it('should confirmation a objective', async () => {
    const lastRecordIndex = (await listPage.table.records.count()) - 1;
    const actionsMenu = listPage.table.getActionsBtn(lastRecordIndex);
    ...
    expect(await deletePage.title.getText()).toEqual('Delete objective');

...
import { FindingCategoryListPage } from './finding-category-list.po';
import { SidenavPage } from '../sidenav.po';
import { FindingCategoryDetailPage } from './finding-category-detail.po';
import { FindingCategoryDeletePage } from './finding-category-delete.po';

describe('FindingCategory tests', () => {
  let headerPage: HeaderPage;
  let loginPage: LoginPage;
  let sidenavPage: SidenavPage;
  let listPage: FindingCategoryListPage;
  let detailPage: FindingCategoryDetailPage;
  let deletePage: FindingCategoryDeletePage;
  ...
  beforeEach(() => {
    headerPage = new HeaderPage();
    sidenavPage = new SidenavPage();
    listPage = new FindingCategoryListPage();
    detailPage = new FindingCategoryDetailPage();
    deletePage = new FindingCategoryDeletePage();
  });

  beforeEach(async () => {
    await waitAndClick(headerPage.appMenu);
    await waitAndClick(sidenavPage.findingCategoryMenu);
  });
  ...
  it('should display the finding-categorys list page', async () => {
    expect(await listPage.getPageTitleText()).toEqual('FindingCategorys');
  ...
  it('should create a new finding-category', async () => {
    await listPage.createBtn.click();

    expect(await detailPage.pageTitle.getText()).toEqual('FindingCategory');
    expect(await detailPage.pageSubTitle.getText()).toEqual(
      'Creates a new findingCategory.'
    );
  ...
  it('should update finding-category', async () => {
    ...
    expect(await detailPage.pageTitle.getText()).toEqual('FindingCategory');
    expect(await detailPage.pageSubTitle.getText()).toEqual(
      'Update an existing findingCategory.'
    );
    expect(await detailPage.cancelBtn.isEnabled()).toBeTruthy();
    expect(await detailPage.saveBtn.isEnabled()).toBeTruthy();
  ...
  it('should delete a finding-category', async () => {
    ...
    expect(await deletePage.title.getText()).toEqual('Delete finding-category');

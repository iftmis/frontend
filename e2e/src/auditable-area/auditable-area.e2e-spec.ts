...
import { AuditableAreaListPage } from './auditable-area-list.po';
import { SidenavPage } from '../sidenav.po';
import { AuditableAreaDetailPage } from './auditable-area-detail.po';
import { AuditableAreaDeletePage } from './auditable-area-delete.po';

describe('AuditableArea tests', () => {
  let headerPage: HeaderPage;
  let loginPage: LoginPage;
  let sidenavPage: SidenavPage;
  let listPage: AuditableAreaListPage;
  let detailPage: AuditableAreaDetailPage;
  let deletePage: AuditableAreaDeletePage;
  ...
  beforeEach(() => {
    headerPage = new HeaderPage();
    sidenavPage = new SidenavPage();
    listPage = new AuditableAreaListPage();
    detailPage = new AuditableAreaDetailPage();
    deletePage = new AuditableAreaDeletePage();
  });

  beforeEach(async () => {
    await waitAndClick(headerPage.appMenu);
    await waitAndClick(sidenavPage.auditableAreaMenu);
  });
  ...
  it('should display the auditable-areas list page', async () => {
    expect(await listPage.getPageTitleText()).toEqual('AuditableAreas');
  ...
  it('should create a new auditable-area', async () => {
    await listPage.createBtn.click();

    expect(await detailPage.pageTitle.getText()).toEqual('AuditableArea');
    expect(await detailPage.pageSubTitle.getText()).toEqual(
      'Creates a new auditableArea.'
    );
  ...
  it('should update auditable-area', async () => {
    ...
    expect(await detailPage.pageTitle.getText()).toEqual('AuditableArea');
    expect(await detailPage.pageSubTitle.getText()).toEqual(
      'Update an existing auditableArea.'
    );
    expect(await detailPage.cancelBtn.isEnabled()).toBeTruthy();
    expect(await detailPage.saveBtn.isEnabled()).toBeTruthy();
  ...
  it('should confirmation a auditable-area', async () => {
    ...
    expect(await deletePage.title.getText()).toEqual('Delete auditable-area');
